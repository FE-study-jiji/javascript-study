...
5.11에서 본 것처럼, 화살표 함수에서 `super`에 접근하면 `super`를 외부 함수에서 가져옵니다.
...
클래스가 다른 클래스를 상속받고 자체 `constructor`가 없는 경우엔 **‘비어있는’ `constructor`가 자동으로 만들어집니다.**
...
만일 메서드를 오버라이딩했다면, 자식 클래스에서 부모 생성자를 호출했을 때 오버라이딩한 메서드가 잘 실행됐을 겁니다.
왜 차이가 있을까요?
...
이 때문에 자바스크립트는 오버라이딩시 필드와 메서드의 동작 방식이 미묘하게 다릅니다.
다행히도 이런 문제는 오버라이딩한 필드를 부모 생성자에서 사용할 때만 발생하지만,
이 차이를 모르면 결과를 해석할 수 없는 상황이 발생할 수 있습니다.
...
`super`에 대해서 좀 더 깊이 파봅시다. 

먼저, 지금까지 배운 내용만으론 `super`가 제대로 동작하지 않습니다.
내부에서 `super`는 ‘어떻게’ 동작할까요?
"객체 메서드가 실행되면 현재 객체가 `this`가 된다. 이 상태에서 `super.method()`를 호출하면 엔진은 현재 객체의 프로토타입에서 `method`를 찾아야 한다"
는 과정은 정확히 '어떻게' 일어나는 걸까요?

엔진은 현재 객체 `this`를 알기 때문에 `this.__proto__.method`로 부모 객체의 `method`를 찾을 수 있을 것 같지만, 그렇게 간단하지 않습니다.

간결성을 위해 클래스가 아닌 일반 객체로 예시를 들겠습니다.  
체인이 하나일 때는 잘 동작하는 것 같지만, 두개 이상일 때 문제가 발생하기 시작합니다.
...
`(*)`의 `this.__proto__.eat.call(this)`는 `eat`을 프로토타입(`animal`)에서 가져오고 현재 객체의 컨텍스트에 기반하여 `eat`을 호출합니다.
(`.call(this)`가 없다면 현재 객체가 아닌 프로토타입의 컨텍스트에서 부모 `eat`을 실행하니 꼭 필요합니다.)

핵심은 모든 객체 메서드는 **프로토타입 등이 아닌 현재 객체를 this로 갖는다**는 점입니다.
